...
export class RemoteService {
  ...
  baseUrl = 'http://tpk-preprod-ix5.g7.local:8890/interface1/';
  ...
  fetchCourses(status?: Status, zone?: number): Observable<Course[]> {
    ...
    return this.http.post<SearchCourseResponse>(path, body)
      .pipe(
        ...
        mergeAll(),
        //  tap ( course => { console.log('Course trouvée: ' + course.address.formatted); }),
        mergeMap(item => this.getLatLng(item)),
        // filter(element => element.lat != null && element.lng != null),
        toArray()
      );
  }
